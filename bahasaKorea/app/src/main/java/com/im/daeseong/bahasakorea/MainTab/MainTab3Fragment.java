...
public class MainTab3Fragment extends Fragment {
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        //API
        //nTotalPageCount = kataApi.getInstance().size();
    ...
    private void CurrentKata()
    {
        try
        {
            ...
            tv1.setText(item.getKataKor());
            tv2.setText(item.getKataIndo());
            tv3.setText(item.getKataEng());

            CounterTimerLabel();

            //현재 프레그멘트 활성화시만 음성 출력
            if(bActivityFragment){
                PlaySuaraSound(item.getKataKor());
            }


            //API
            /*
            if (CurrentPlayIndex < 0 || kataApi.getInstance().size() == 0) return;

            item = kataApi.getInstance().get(CurrentPlayIndex);
    ...
    private  void PreKata()
    {
        try
        {
            ...
            //API
            /*
            if (kataApi.getInstance().size() == 0) return;
            ...
            if (CurrentPlayIndex < 0)
                CurrentPlayIndex = kataApi.getInstance().size() - 1;
    ...
    private void NextKata()
    {
        try
        {
            ...
            AnimatorUtil.AnimatoSwipeLeft(swipeConstraintLayout);//await SwipeAnimateEffect(false);
            ...
            if (CurrentPlayIndex > (kataApi.getInstance().size() - 1))
                CurrentPlayIndex = 0;
